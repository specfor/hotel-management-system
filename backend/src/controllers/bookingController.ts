...
import { 
  ...
  deleteBookingModel} from "@src/models/bookingModel";
  ...
  try {
    ...
    if (isNaN(guestIDInt)) {
      return jsonResponse(
        ...
        false, 
        HttpStatusCodes.BAD_REQUEST, 
        { message: "Guest ID must be a valid number" });
    }

    const bookings = await getAllBookingsModel({ guestId: guestIDInt });
...
export async function getBookingsByRoomID(req: Request, res: Response) {
  try {
    ...
    const bookings = await getAllBookingsModel({ roomId: roomIDInt });
  ...
  try {
    ...
    if (
      ...
      return jsonResponse(
        ...
        false,
        HttpStatusCodes.BAD_REQUEST,
        { message: "Invalid format for roomID or dates" });
    }

    const conflictingBookings = await getConflictingBookingsModel(
      roomIDInt,
      checkInDate, 
      checkOutDate);
...
export async function createBooking(req: Request, res: Response) {
  try {
    const { userId, guestId, roomId, checkIn, checkOut } = req.body as {
      userId: number,
      guestId: number,
      roomId: number,
      checkIn: string,
