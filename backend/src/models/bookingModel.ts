...
import { 
  ...
  getBookingByIDDB,
  deleteBookingDB} from "@src/repos/bookingRepo"; 
...
//   1. Joi Validation: 
  ...
  if (error) { throw new RouteError(HttpStatusCodes.BAD_REQUEST, error.details[0].message); }
  ...
  if (conflicts && conflicts.length > 0) {
    ...
    throw new RouteError(
      ...
      "Room is unavailable due to conflicting booking(s).",
    );
  }
...
export async function getConflictingBookingsModel(
  roomId: number, 
  checkIn: Date,
  checkOut: Date): Promise<BookingPublic[]> {
